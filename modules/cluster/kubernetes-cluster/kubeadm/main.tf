...
resource "kubernetes_manifest" "control_plane" {
  ...
  manifest = local.cluster_manifests[each.key]["KubeadmControlPlane-${each.value.name}-control-plane"]
...
resource "kubernetes_manifest" "worker_config_template" {
  ...
  manifest = local.cluster_manifests[each.key]["KubeadmConfigTemplate-${each.value.name}-worker-config"]
