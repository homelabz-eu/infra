  ...
  template_files = [
    {
      ...
      vars = {
        ...
        k0s_control_plane_name = "${each.value.name}-k0s-cp"
    ...
    },
    {
      ...
      vars = {
        cluster_name           = each.value.name
        namespace              = each.value.name
        worker_k0s_config_name = "${each.value.name}-worker-config"
        ssh_authorized_keys    = jsonencode(var.ssh_authorized_keys)
      }
    },
    {
...
resource "kubernetes_manifest" "control_plane" {
  ...
  manifest = local.cluster_manifests[each.key]["K0sControlPlane-${each.value.name}-k0s-cp"]
...
resource "kubernetes_manifest" "worker_config_template" {
  ...
  manifest = local.cluster_manifests[each.key]["K0sWorkerConfigTemplate-${each.value.name}-worker-config"]
